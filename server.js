 ...
 * Handles audio file uploads, transcribes them using Google Cloud Speech-to-Text,
...
app.post("/stt", upload.single("audio"), async (req, res) => {
    ...
    // Check if the Google Speech client was initialized correctly
    if (!sttClient) {
        return res.status(500).json({ error: "Google Speech client not initialized." });
    }

    // Get the language code from the request, defaulting to English (India)
    const languageCode = req.body.languageCode || "en-IN";

    try {
        // 1. Read the audio file from the temporary path where multer saved it
        const audioBytes = fs.readFileSync(req.file.path).toString("base64");

        // 2. Configure the request for the Speech-to-Text API
        // These settings should match how your Android app records the audio
        const config = {
            encoding: "AMR",
            sampleRateHertz: 8000,
            languageCode: languageCode, // Use the language sent from the app
            alternativeLanguageCodes: ["en-IN", "hi-IN", "pa-IN"], // Other languages to try
        };

        const audio = { content: audioBytes };
        const request = { audio, config };

        // 3. Call the Google Cloud API to recognize the speech
        const [response] = await sttClient.recognize(request);
        
        // 4. Extract the most likely transcription from the response
        const transcription =
            response.results?.map((r) => r.alternatives[0].transcript).join("\n") || "";

        // 5. Send the transcribed text back to the Android app
        ...
        // 6. IMPORTANT: Always delete the temporary file after processing
...
app.post("/chat", async (req, res) => {
    ...
    try {
        ...
        const systemPrompt = `You are RozgarAI, a helpful AI career advisor. You MUST use the 'find_jobs' tool to answer job-related questions. When presenting jobs, you MUST use the exact data from the tool. Do not invent details. Guide users to the application link for more information. ${personalizationContext} ${languageInstruction}`;
